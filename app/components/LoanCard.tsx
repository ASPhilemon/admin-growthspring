...
function getInterestAndPoints(loan:any){
  ...
  let points_accrued =  0;
    ...
    const loanYear = loan_finding.loan_date.getFullYear();
        ...
        let current_principal_duration = getDaysDifference(loan_finding.last_payment_date, new Date()) % 30 < 0.24 ? Math.trunc(getDaysDifference(loan_finding.last_payment_date, new Date()) / 30): Math.ceil(getDaysDifference(loan_finding.last_payment_date, new Date()) / 30);
